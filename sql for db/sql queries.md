as,distinct,orderby,where,like,between,and,or,(like where %man% means if contins the word and also it is  not case senstive),is null, is not null(is not remeber) use of (## limit) use of (case)
...
## Where (shd no that does for rows only)
...
SELECT * 
FROM movies
WHERE name LIKE 'Se_en'; and also has % 
...
## Is Null(cant be compared with != and =)
...
## Between( in alpha it is alpha check and one letter ka )
between a and d means will see startng letter ka from a,b,c and d is only d letter movies or else wont include but in numbers the 2nd wala is also included
...
## Order By(after where and also dont need to be the one which is being displayed and use of desc and aesc)
...
## Limit(always at last in writing)
