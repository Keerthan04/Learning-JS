//can access the method of request like get,post and all from the req.method
...
/*
  ...
  2.DRYing Code With Functions(Dont repeat yourself(DRY),dont violate DRY)
  ...
  3.DRYing Routes With app.use()(app.use executed whenever a request is made to the server)
  ...
  5.Request And Response Parameters(all middleware have acces to res.req and next middleware)
  ...
  6.Route-Level app.use() - Single Path(path mentioned in app.use means will work for that only like the mounting of routers)
    ...
    //if whenever someone visits a path then we need to check for something so instead of writing it in all routes we write in app.use at specified path so that we can use it in all routes(it will be done in all the routes or first when the call is made to that route) and all used if we want to send something of all the routes of that then we add to req as a new property and send
    ex
  ...
  8.Route-Level app.use() - Multiple Paths(can take optional multiple paths also)
    ...
    //means we dont need to add something that happens for all routes of that path and if same for diff path separetly we combine it and write in one app.use only(can be for diff paths or same path diff types like diff path names type and all also)

    example:
    app.use(['/beans/', '/beans/:beanName'], (req, res, next) => {//see how array of paths
        let bodyData = '';
        req.on('data', (data) => {
            bodyData += data;
        });
    ...
    IMPORTANT
    //the req.on is to do that when the request is completely got so see it will do like when data is received
    it makes the data as a string fully
    then once all are recieved in .on('end) if the bodydat is there it will parse it into a json and make it as a property of req body so in all the routes they can access the req.body and it will have all the things we made it to be ther in req.body and then next called

    //if just app.use((....)) the the middleware is for all paths i.e whenever a request is done it works

  9.Middleware Stacks(use of multiple callbacks,func define and pass okay)
    ...
    //means create our function and pass them with normal callback in use and all
    
  10.Open-Source Middleware: Logging
    ...
    //logging middleware means which is used for logging(console.log)
    morgan('tiny') or any other para will send the req,res,next middleware only so used instead of that but will log that the response is sent so everytime we send response we will log that
  ...
  12.Open-Source Middleware: Body Parsing
